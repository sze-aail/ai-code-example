...
def main():
    ...
    # Larger dataset: 64x64 identity matrix
    X = torch.eye(64)
    y = torch.eye(64)

    # Larger model: 64 -> 32 -> 64
    model = nn.Sequential(
        nn.Linear(64, 32, bias=False),  # W1: 32x64
        nn.Tanh(),
        nn.Linear(32, 64, bias=False)   # W2: 64x32
    )
    ...
    def update(frame_idx):
        ...
        ax_w1.set_title("W1 Súlyok (32x64)")

        im_w2 = ax_w2.imshow(data['w2'], cmap=cmap, vmin=-vmax_w, vmax=vmax_w)
        ax_w2.set_title("W2 Súlyok (64x32)")
        ...
        fig.suptitle(f"Nagyobb háló Súlyok/Gradiensek\nEpocha: {data['epoch']} | Loss: {data['loss']:.4f}", fontsize=16)

    print("Animáció készítése...")
    anim = FuncAnimation(fig, update, frames=len(frames), blit=False)

    output_file = "05_weight_tensor_animation_large.gif"
